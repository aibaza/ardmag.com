...
import type { CreateShippingOptionDTO, Logger } from "@medusajs/framework/types"
import { computeTotalWeightKg, getItemsTotalRon, ShippingContextItem } from "../fulfillment-shared/weight"
...
export class CargusProviderService extends AbstractFulfillmentProviderService {
  ...
  async getFulfillmentOptions() { return [{ id: "cargus-standard" }] }
  async validateOption(_data: Record<string, unknown>) { return true }
  async canCalculate(_data: CreateShippingOptionDTO) { return true }
  ...
  async calculatePrice(
    ...
    const address = context.shipping_address as { province?: string; city?: string } | undefined
    const totalWeightKg = await computeTotalWeightKg(items, (error) => {
      this.logger.warn(`[Cargus] variant weight lookup failed: ${error.message}`)
    })
    const itemTotal = getItemsTotalRon(items)

    if (itemTotal >= FREE_SHIPPING_THRESHOLD_RON) {
      this.logger.info(`[Cargus] quote source=free-shipping weight=${totalWeightKg}kg`)
      return { calculated_amount: 0, is_calculated_price_tax_inclusive: true }
    }

    if (!process.env.CARGUS_API_KEY) {
      this.logger.info(`[Cargus] quote source=static-fallback reason=key-unset`)
      return {
    ...
    try {
      ...
      this.logger.info(`[Cargus] quote source=cargus-api latency_ms=${Date.now() - startedAt}`)
      return { calculated_amount: amount, is_calculated_price_tax_inclusive: true }
    } catch (error) {
      this.logger.warn(
        `[Cargus] quote source=static-fallback latency_ms=${Date.now() - startedAt} error=${(error as Error).name}`
      )
      return {
        calculated_amount: fallbackTariff(totalWeightKg),
        is_calculated_price_tax_inclusive: true,
  ...
  async createFulfillment() { return { data: {}, labels: [] } }
  async cancelFulfillment() { return {} }
  async createReturnFulfillment() { return { data: {}, labels: [] } }
}
