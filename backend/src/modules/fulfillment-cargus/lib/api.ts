const BASE_URL =
  process.env.CARGUS_API_URL || "https://urgentcargus.azure-api.net/api";
...
export async function quoteCargus(input: CargusQuoteRequest): Promise<number> {
  ...
  if (!response.ok)
    throw new Error(`Cargus quote failed with status ${response.status}`);

  const body = (await response.json()) as Record<string, unknown>;
  const total = Number(body.GrandTotal ?? body.grandTotal ?? body.total);
  if (!Number.isFinite(total) || total < 0)
    throw new Error("Cargus quote total is invalid");
