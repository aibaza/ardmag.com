...
describe("delivery-payment-policy", () => {
  ...
  const pickup = { code: "pickup-cluj", providerId: "manual_manual", name: "Ridicare Cluj" }
  ...
  it("permite Fan doar cu ramburs", () => {
    expect(validateDeliveryPaymentPair(fan, "pp_system_default").valid).toBe(true)
    expect(validateDeliveryPaymentPair(fan, "pp_stripe_stripe").valid).toBe(false)
  })

  it("permite Cargus doar cu Stripe card", () => {
    expect(validateDeliveryPaymentPair(cargus, "pp_stripe_stripe").valid).toBe(true)
    expect(validateDeliveryPaymentPair(cargus, "pp_system_default").valid).toBe(false)
    expect(isStripeCardProvider("pp_stripe_ideal")).toBe(false)
  })

  it("nu schimba politica pentru Ridicare Cluj", () => {
    expect(validateDeliveryPaymentPair(pickup, "pp_system_default").valid).toBe(true)
    expect(validateDeliveryPaymentPair(pickup, "pp_stripe_stripe").valid).toBe(true)
  })
})
