...
async function getShippingOption(req: MedusaRequest, shippingOptionId?: string | null) {
...
export async function getCartPolicyState(req: MedusaRequest, cartId: string): Promise<PolicyState> {
  const query = req.scope.resolve(ContainerRegistrationKeys.QUERY)
  const { data } = await query.graph({
    entity: "cart",
    fields: [
  ...
  const option = await getShippingOption(req, shippingMethod?.shipping_option_id)
  const sessions = cart?.payment_collection?.payment_sessions ?? []
  const session = sessions.find((item: any) => item.status !== "canceled") ?? sessions[0]
...
export function policyError(state: PolicyState, providerId?: string | null): string | null {
  const result = validateDeliveryPaymentPair(state.delivery, providerId ?? state.paymentProviderId)
