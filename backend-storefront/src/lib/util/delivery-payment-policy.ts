export type StorefrontDeliveryKind =
  | "fan-courier"
  | "cargus"
  | "pickup-cluj"
  | "other"
...
export function deliveryKind(
  option?: ShippingLike | null
): StorefrontDeliveryKind {
  const values = [option?.type?.code, option?.provider_id, option?.name]
    .filter(Boolean)
    .map((value) => String(value).toLowerCase())
  if (
    values.some(
      (value) => value.includes("fan-courier") || value.includes("fan courier")
    )
  )
    return "fan-courier"
  ...
  if (
    values.some(
      (value) =>
        value.includes("pickup-cluj") || value.includes("ridicare cluj")
    )
  )
    return "pickup-cluj"
  return "other"
...
export function paymentAllowedForDelivery(
  kind: StorefrontDeliveryKind,
  providerId: string
): boolean {
...
export function deliveryPaymentError(
  kind: StorefrontDeliveryKind,
