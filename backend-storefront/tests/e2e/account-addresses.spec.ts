...
test("Account addresses: CRUD complet + default flags", async ({ page, request }) => {
  ...
  await expect(page.locator("h4", { hasText: /adresa noua/i })).toBeVisible()
  ...
  await page.check('[name="is_default_shipping"]')
  await page.check('[name="is_default_billing"]')
  await page.locator("button", { hasText: /salveaza adresa/i }).click()

  await expect(page.locator("h4", { hasText: /adresa noua/i })).not.toBeVisible({ timeout: 10000 })
  ...
  await expect(page.locator("h4", { hasText: /editeaza adresa/i })).toBeVisible()
  await page.locator('[name="phone"]').fill("0733999999")
  await page.locator("button", { hasText: /^salveaza$/i }).click()
  await expect(page.locator("h4", { hasText: /editeaza adresa/i })).not.toBeVisible({ timeout: 10000 })
